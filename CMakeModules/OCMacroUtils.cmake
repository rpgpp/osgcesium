...
MACRO(LINK_INTERNAL TRGTNAME)
    ...
    IF(WIN32)
        TARGET_LINK_LIBRARIES(${TRGTNAME} optimized "${LINKLIB}${CMAKE_RELEASE_POSTFIX}.lib" debug "${LINKLIB}${CMAKE_DEBUG_POSTFIX}.lib")
    ELSE(WIN32)
...
MACRO(SETUP_LIBRARY LIB_NAME)
    ...
    ENDIF()
        ...
        SET_TARGET_PROPERTIES(${LIB_NAME} PROPERTIES FOLDER "OC Core")
        IF(APPLE)
...
MACRO(SETUP_GROUP)
    if(MSVC)
        foreach(file_path ${TARGET_H})
        string(REGEX MATCH ".+\/.+\.h" inFolder ${file_path})
        if(inFolder)
            string(REGEX REPLACE "(.+)\/.+h" \\1 folder_name ${file_path})
            source_group(${folder_name} FILES ${file_path})
        endif(inFolder)
        endforeach(file_path)

        foreach(file_path ${TARGET_SRC})
        string(REGEX MATCH ".+\/.+c*" inFolder ${file_path})
        if(inFolder)
            string(REGEX REPLACE "(.+)\/.+c*" \\1 folder_name ${file_path})
            string(REGEX MATCH "src" issrc ${folder_name})
            if(issrc STREQUAL "")
                source_group(${folder_name} FILES ${file_path})
            endif()
        endif()
