...
{
public:
    std::string _gbaGetModulePath()
    {
#if _DEBUG
        HANDLE hDllhandle = GetModuleHandleA("osgdb_wrld.dll");
#else
        HANDLE hDllhandle = GetModuleHandleA("osgdb_wrl.dll");
#endif
        char filename[MAX_PATH + 1];
        std::string strRtn = "";
        if (GetModuleFileNameA((HMODULE)hDllhandle, filename, MAX_PATH))
        {
            strRtn = filename;
        }
        return strRtn;
    ...
    ReaderWriterVRML2()
    {
        std::string dir = osgDB::getFilePath(_gbaGetModulePath());
        std::string node = "OPENVRML_NODE_PATH=" + dir + "/../vrml/node";
        std::string data = "OPENVRML_DATADIR=" + dir + "/../vrml/data";
        putenv(node.c_str());
        putenv(data.c_str());
