...
namespace OC
{
	...
	inline void pushSplitRequest(ConvertWorkqueue& workqueue, ObliqueConverter* converter, String destDir, osg::Node* node, OC::Cesium::Tile* parentTile,int code)
	{
		ConvertWorkqueue::TaskRequest request;
	...
	void ObliqueConverter::split(osg::Group* node, OC::Cesium::Tile* parentTile,bool shouldSplit ,int depth,String destDir,int code)
	{
		...
		for (int i = 0; i < (int)splitGroup->getNumChildren(); i++)
		{
			...
			osg::ref_ptr<OC::Cesium::Tile> tile = new OC::Cesium::Tile;
			parentTile->children().push_back(tile);
	...
	void ObliqueConverter::readNode(String directory)
	{
		...
		osg::ref_ptr<TDTILES::Tileset> tileset = OC_NEW_Tiltset();
		tileset->root()->transform() = MapProject::computeTransform(mLocation);
