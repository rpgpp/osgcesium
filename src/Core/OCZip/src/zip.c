...
extern zipFile ZEXPORT zipOpenWin32(const char* pathname, int append)
{
    #if _WIN32
    zlib_filefunc_def ffunc;
    fill_win32_filefunc(&ffunc);
    return zipOpen2(pathname, append, NULL, &ffunc);
    #else
    return 0L;
    #endif
}

extern zipFile ZEXPORT zipOpenStd(const char* pathname, int append, voidp stream)
{
    zlib_filefunc_def ffunc;
    fill_std_filefunc(&ffunc);
    ffunc.opaque = stream;
    return zipOpen2(pathname, append, NULL, &ffunc);
}
