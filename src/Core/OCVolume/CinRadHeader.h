...
struct CINRAD_RecordCommon
{
	CINRAD_Record header;
	std::vector<unsigned char> bufferDBZ;
	std::vector<unsigned char> bufferDoppler;
	std::vector<unsigned char> bufferSpectralWidth;
	char reserve5[4];
};
...
struct CINRAD_RecordFMT_CutConfig
{
	...
	float nyquistSpeed;
	unsigned int momentsMask;
	unsigned int momentsSizeMask;
	int miscFileterMask;
	float sqiThreshold;
	float sigThreshold;
	float csrThreshold;
