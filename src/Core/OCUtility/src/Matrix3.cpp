...
namespace OC
{
    const Real CMatrix3::EPSILON = 1e-06;
    const CMatrix3 CMatrix3::ZERO(0,0,0,0,0,0,0,0,0);
    const CMatrix3 CMatrix3::IDENTITY(1,0,0,0,1,0,0,0,1);
    const Real CMatrix3::msSvdEpsilon = 1e-04;
    const unsigned int CMatrix3::msSvdMaxIterations = 32;

    //-----------------------------------------------------------------------
    CVector3 CMatrix3::GetColumn (size_t iCol) const
    {
        assert( iCol < 3 );
        return CVector3(m[0][iCol],m[1][iCol],
            m[2][iCol]);
    }
    //-----------------------------------------------------------------------
    void CMatrix3::SetColumn(size_t iCol, const CVector3& vec)
    {
        assert( iCol < 3 );
        m[0][iCol] = vec.x;
        m[1][iCol] = vec.y;
    ...
    //-----------------------------------------------------------------------
    void CMatrix3::FromAxes(const CVector3& xAxis, const CVector3& yAxis, const CVector3& zAxis)
    {
        SetColumn(0,xAxis);
        SetColumn(1,yAxis);
        SetColumn(2,zAxis);
    ...
    //-----------------------------------------------------------------------
    bool CMatrix3::operator== (const CMatrix3& rkMatrix) const
    {
    ...
    //-----------------------------------------------------------------------
    CMatrix3 CMatrix3::operator+ (const CMatrix3& rkMatrix) const
    {
        CMatrix3 kSum;
        for (size_t iRow = 0; iRow < 3; iRow++)
        {
    ...
    //-----------------------------------------------------------------------
    CVector3 CMatrix3::operator* (const CVector3& rkPoint) const
    {
        CVector3 kProd;
        for (size_t iRow = 0; iRow < 3; iRow++)
        {
    ...
    //-----------------------------------------------------------------------
    CVector3 operator* (const CVector3& rkPoint, const CMatrix3& rkMatrix)
    {
        CVector3 kProd;
        for (size_t iRow = 0; iRow < 3; iRow++)
        {
    ...
    //-----------------------------------------------------------------------
    CMatrix3 CMatrix3::operator* (Real fScalar) const
    {
        CMatrix3 kProd;
        for (size_t iRow = 0; iRow < 3; iRow++)
        {
    ...
    //-----------------------------------------------------------------------
    CMatrix3 operator* (Real fScalar, const CMatrix3& rkMatrix)
    {
        CMatrix3 kProd;
        for (size_t iRow = 0; iRow < 3; iRow++)
        {
    ...
    //-----------------------------------------------------------------------
    bool CMatrix3::Inverse (CMatrix3& rkInverse, Real fTolerance) const
    {
    ...
    //-----------------------------------------------------------------------
    CMatrix3 CMatrix3::Inverse (Real fTolerance) const
    {
        CMatrix3 kInverse = CMatrix3::ZERO;
        Inverse(kInverse,fTolerance);
        return kInverse;
    }
    //-----------------------------------------------------------------------
    Real CMatrix3::Determinant () const
    {
    ...
    //-----------------------------------------------------------------------
    void CMatrix3::Bidiagonalize (CMatrix3& kA, CMatrix3& kL,
        CMatrix3& kR)
    {
        Real afV[3], afW[3];
    ...
    //-----------------------------------------------------------------------
    void CMatrix3::GolubKahanStep (CMatrix3& kA, CMatrix3& kL,
        CMatrix3& kR)
    {
    ...
    //-----------------------------------------------------------------------
    void CMatrix3::SingularValueDecomposition (CMatrix3& kL, CVector3& kS,
        CMatrix3& kR) const
    {
    ...
    //-----------------------------------------------------------------------
    void CMatrix3::SingularValueComposition (const CMatrix3& kL,
        const CVector3& kS, const CMatrix3& kR)
    {
        size_t iRow, iCol;
        CMatrix3 kTmp;
    ...
    //-----------------------------------------------------------------------
    void CMatrix3::QDUDecomposition (CMatrix3& kQ,
        CVector3& kD, CVector3& kU) const
    {
		...
		if (!OC::Math::RealEqual(fInvLength,0)) fInvLength = Math::InvSqrt(fInvLength);
    ...
    //-----------------------------------------------------------------------
    Real CMatrix3::MaxCubicRoot (Real afCoeff[3])
    {
    ...
    //-----------------------------------------------------------------------
    Real CMatrix3::SpectralNorm () const
    {
        CMatrix3 kP;
        size_t iRow, iCol;
        Real fPmax = 0.0;
    ...
    //-----------------------------------------------------------------------
    void CMatrix3::ToAngleAxis (CVector3& rkAxis, CRadian& rfRadians) const
    {
        ...
        if ( rfRadians > CRadian(0.0) )
        {
            if ( rfRadians < CRadian(Math::PI) )
            {
    ...
    //-----------------------------------------------------------------------
    void CMatrix3::FromAngleAxis (const CVector3& rkAxis, const CRadian& fRadians)
    {
    ...
    //-----------------------------------------------------------------------
    bool CMatrix3::ToEulerAnglesXYZ (CRadian& rfYAngle, CRadian& rfPAngle,
        CRadian& rfRAngle) const
    {
        ...
        rfPAngle = CRadian(Math::ASin(m[0][2]));
        if ( rfPAngle < CRadian(Math::HALF_PI) )
        {
            if ( rfPAngle > CRadian(-Math::HALF_PI) )
            {
            ...
            else
            {
                ...
                CRadian fRmY = Math::ATan2(m[1][0],m[1][1]);
                rfRAngle = CRadian(0.0);  // any angle works
        ...
        {
            // WARNING.  Not a unique solution.
            CRadian fRpY = Math::ATan2(m[1][0],m[1][1]);
            rfRAngle = CRadian(0.0);  // any angle works
    ...
    //-----------------------------------------------------------------------
    bool CMatrix3::ToEulerAnglesXZY (CRadian& rfYAngle, CRadian& rfPAngle,
        CRadian& rfRAngle) const
    {
        // rot =  cy*cz          -sz              cz*sy
        ...
        if ( rfPAngle < CRadian(Math::HALF_PI) )
        {
            ...
            {
                // WARNING.  Not a unique solution.
                CRadian fRmY = Math::ATan2(-m[2][0],m[2][2]);
                rfRAngle = CRadian(0.0);  // any angle works
                rfYAngle = rfRAngle - fRmY;
                return false;
            }
        ...
        {
            // WARNING.  Not a unique solution.
            CRadian fRpY = Math::ATan2(-m[2][0],m[2][2]);
            rfRAngle = CRadian(0.0);  // any angle works
            rfYAngle = fRpY - rfRAngle;
            return false;
        }
    ...
    //-----------------------------------------------------------------------
    bool CMatrix3::ToEulerAnglesYXZ (CRadian& rfYAngle, CRadian& rfPAngle,
        CRadian& rfRAngle) const
    {
        ...
        if ( rfPAngle < CRadian(Math::HALF_PI) )
        {
            ...
            {
                // WARNING.  Not a unique solution.
                CRadian fRmY = Math::ATan2(-m[0][1],m[0][0]);
                rfRAngle = CRadian(0.0);  // any angle works
                rfYAngle = rfRAngle - fRmY;
                return false;
            }
        ...
        {
            // WARNING.  Not a unique solution.
            CRadian fRpY = Math::ATan2(-m[0][1],m[0][0]);
            rfRAngle = CRadian(0.0);  // any angle works
            rfYAngle = fRpY - rfRAngle;
            return false;
        }
    ...
    //-----------------------------------------------------------------------
    bool CMatrix3::ToEulerAnglesYZX (CRadian& rfYAngle, CRadian& rfPAngle,
        CRadian& rfRAngle) const
    {
        ...
        if ( rfPAngle < CRadian(Math::HALF_PI) )
        {
            ...
            {
                // WARNING.  Not a unique solution.
                CRadian fRmY = Math::ATan2(m[2][1],m[2][2]);
                rfRAngle = CRadian(0.0);  // any angle works
                rfYAngle = rfRAngle - fRmY;
                return false;
            }
        ...
        {
            // WARNING.  Not a unique solution.
            CRadian fRpY = Math::ATan2(m[2][1],m[2][2]);
            rfRAngle = CRadian(0.0);  // any angle works
            rfYAngle = fRpY - rfRAngle;
            return false;
        }
    ...
    //-----------------------------------------------------------------------
    bool CMatrix3::ToEulerAnglesZXY (CRadian& rfYAngle, CRadian& rfPAngle,
        CRadian& rfRAngle) const
    {
        ...
        if ( rfPAngle < CRadian(Math::HALF_PI) )
        {
            ...
            {
                // WARNING.  Not a unique solution.
                CRadian fRmY = Math::ATan2(m[0][2],m[0][0]);
                rfRAngle = CRadian(0.0);  // any angle works
                rfYAngle = rfRAngle - fRmY;
                return false;
            }
        ...
        {
            // WARNING.  Not a unique solution.
            CRadian fRpY = Math::ATan2(m[0][2],m[0][0]);
            rfRAngle = CRadian(0.0);  // any angle works
            rfYAngle = fRpY - rfRAngle;
            return false;
        }
    ...
    //-----------------------------------------------------------------------
    bool CMatrix3::ToEulerAnglesZYX (CRadian& rfYAngle, CRadian& rfPAngle,
        CRadian& rfRAngle) const
    {
        ...
        if ( rfPAngle < CRadian(Math::HALF_PI) )
        {
            ...
            {
                // WARNING.  Not a unique solution.
                CRadian fRmY = Math::ATan2(-m[0][1],m[0][2]);
                rfRAngle = CRadian(0.0);  // any angle works
                rfYAngle = rfRAngle - fRmY;
                return false;
            }
        ...
        {
            // WARNING.  Not a unique solution.
            CRadian fRpY = Math::ATan2(-m[0][1],m[0][2]);
            rfRAngle = CRadian(0.0);  // any angle works
            rfYAngle = fRpY - rfRAngle;
            return false;
        }
    ...
    //-----------------------------------------------------------------------
    void CMatrix3::FromEulerAnglesXYZ (const CRadian& fYAngle, const CRadian& fPAngle,
        const CRadian& fRAngle)
    {
        Real fCos, fSin;
    ...
    //-----------------------------------------------------------------------
    void CMatrix3::FromEulerAnglesXZY (const CRadian& fYAngle, const CRadian& fPAngle,
        const CRadian& fRAngle)
    {
        Real fCos, fSin;
    ...
    //-----------------------------------------------------------------------
    void CMatrix3::FromEulerAnglesYXZ (const CRadian& fYAngle, const CRadian& fPAngle,
        const CRadian& fRAngle)
    {
        Real fCos, fSin;
    ...
    //-----------------------------------------------------------------------
    void CMatrix3::FromEulerAnglesYZX (const CRadian& fYAngle, const CRadian& fPAngle,
        const CRadian& fRAngle)
    {
        Real fCos, fSin;
    ...
    //-----------------------------------------------------------------------
    void CMatrix3::FromEulerAnglesZXY (const CRadian& fYAngle, const CRadian& fPAngle,
        const CRadian& fRAngle)
    {
        Real fCos, fSin;
    ...
    //-----------------------------------------------------------------------
    void CMatrix3::FromEulerAnglesZYX (const CRadian& fYAngle, const CRadian& fPAngle,
        const CRadian& fRAngle)
    {
        Real fCos, fSin;
    ...
    //-----------------------------------------------------------------------
    void CMatrix3::Tridiagonal (Real afDiag[3], Real afSubDiag[3])
    {
    ...
    //-----------------------------------------------------------------------
    bool CMatrix3::QLAlgorithm (Real afDiag[3], Real afSubDiag[3])
    {
    ...
    //-----------------------------------------------------------------------
    void CMatrix3::EigenSolveSymmetric (Real afEigenvalue[3],
        CVector3 akEigenvector[3]) const
    {
        CMatrix3 kMatrix = *this;
        Real afSubDiag[3];
        ...
        CVector3 kCross = akEigenvector[1].crossProduct(akEigenvector[2]);
    ...
    //-----------------------------------------------------------------------
    void CMatrix3::TensorProduct (const CVector3& rkU, const CVector3& rkV,
        CMatrix3& rkProduct)
    {
        for (size_t iRow = 0; iRow < 3; iRow++)
        {
