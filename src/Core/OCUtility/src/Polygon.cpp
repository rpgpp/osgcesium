...
namespace OC
{
	...
	//-----------------------------------------------------------------------
	void Polygon::insertVertex(const CVector3& vdata, size_t vertex )
	{
	...
	//-----------------------------------------------------------------------
	void Polygon::insertVertex(const CVector3& vdata)
	{
		mVertexList.push_back(vdata);
	}
	//-----------------------------------------------------------------------
	const CVector3& Polygon::getVertex( size_t vertex ) const
	{
	...
	//-----------------------------------------------------------------------
	void Polygon::setVertex(const CVector3& vdata, size_t vertex )
	{
		// TODO: optional: check planarity
		//OgreAssert(vertex < getVertexCount(), "Search position out of range" );
	...
	void Polygon::removeDuplicates( void )
	{
		...
		{
			const CVector3& a = getVertex( i );
			const CVector3& b = getVertex( (i + 1)%getVertexCount() );
	...
	//-----------------------------------------------------------------------
	const CVector3& Polygon::getNormal( void ) const
	{
	...
	void Polygon::updateNormal( void ) const
	{
		...
		// vertex order is ccw
		const CVector3& a = getVertex( 0 );
		const CVector3& b = getVertex( 1 );
		const CVector3& c = getVertex( 2 );
	...
	bool Polygon::operator == (const Polygon& rhs) const
	{
		...
		for (size_t i = 0; i < getVertexCount(); ++i )
		{
			const CVector3& vA = getVertex( i );
			const CVector3& vB = rhs.getVertex( ( i + start) % getVertexCount() );
	...
	//-----------------------------------------------------------------------
	bool Polygon::isPointInside(const CVector3& point) const
	{
		// sum the angles 
		Real anglesum = 0;
		...
		{
			const CVector3& p1 = getVertex(i);
			const CVector3& p2 = getVertex((i + 1) % n);

			CVector3 v1 = p1 - point;
			CVector3 v2 = p2 - point;
