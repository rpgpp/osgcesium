...
namespace OC
{
	...
	Math::Math( unsigned int trigTableSize )
	{
		...
		//mSinTable = CORE_ALLOC_T(Real, mTrigTableSize, MEMCATEGORY_GENERAL);
		//mTanTable = CORE_ALLOC_T(Real, mTrigTableSize, MEMCATEGORY_GENERAL);
	...
	Math::~Math()
	{
		//CORE_FREE(mSinTable, MEMCATEGORY_GENERAL);
		//CORE_FREE(mTanTable, MEMCATEGORY_GENERAL);
		delete[] mSinTable;
		delete[] mTanTable;
	}
	...
	//-----------------------------------------------------------------------
	bool Math::pointInTri2D(const CVector2& p, const CVector2& a, 
	...
	//-----------------------------------------------------------------------
	bool Math::pointInTri3D(const CVector3& p, const CVector3& a, 
	...
	//-----------------------------------------------------------------------
	std::pair<bool, Real> Math::intersects(const CRay& ray, const CPlane& plane)
	{
	...
	//-----------------------------------------------------------------------
	std::pair<bool, Real> Math::intersects(const CRay& ray, 
		const vector<CPlane>& planes, bool normalIsOutside)
	{
		list<CPlane> planesList;
		for (vector<CPlane>::const_iterator i = planes.begin(); i != planes.end(); ++i)
		{
	...
	//-----------------------------------------------------------------------
	std::pair<bool, Real> Math::intersects(const CRay& ray, 
		const list<CPlane>& planes, bool normalIsOutside)
	{
		list<CPlane>::const_iterator planeit, planeitend;
		...
		CPlane::Side outside = normalIsOutside ? CPlane::POSITIVE_SIDE : CPlane::NEGATIVE_SIDE;

		for (planeit = planes.begin(); planeit != planeitend; ++planeit)
		{
			const CPlane& plane = *planeit;
			// is origin outside?
	...
	//-----------------------------------------------------------------------
	std::pair<bool, Real> Math::intersects(const CRay& ray, const CSphere& sphere, 
		bool discardInside)
	{
		const CVector3& raydir = ray.getDirection();
		// Adjust ray origin relative to sphere center
		const CVector3& rayorig = ray.getOrigin() - sphere.getCenter();
	...
	//-----------------------------------------------------------------------
	std::pair<bool, Real> Math::intersects(const CRay& ray, const CAxisAlignedBox& box)
	{
		...
		bool hit = false;
		CVector3 hitpoint;
		const CVector3& min = box.getMinimum();
		const CVector3& max = box.getMaximum();
		const CVector3& rayorig = ray.getOrigin();
		const CVector3& raydir = ray.getDirection();
	...
	//-----------------------------------------------------------------------
	bool Math::intersects(const CRay& ray, const CAxisAlignedBox& box,
		Real* d1, Real* d2)
	{
		if (box.isNull())
			return false;
...
#undef _CALC_AXIS
	...
	//-----------------------------------------------------------------------
	std::pair<bool, Real> Math::intersects(const CRay& ray, const CVector3& a,
		const CVector3& b, const CVector3& c, const CVector3& normal,
	...
	std::pair<bool, Real> Math::intersects(const CRay& ray, const CVector3& a,
		...
		bool positiveSide, bool negativeSide)
	{
		CVector3 normal = calculateBasicFaceNormalWithoutNormalize(a, b, c);
		return intersects(ray, a, b, c, normal, positiveSide, negativeSide);
	}
	//-----------------------------------------------------------------------
	bool Math::intersects(const CSphere& sphere, const CAxisAlignedBox& box)
	{
	...
	//-----------------------------------------------------------------------
	bool Math::intersects(const CPlane& plane, const CAxisAlignedBox& box)
	{
		return (plane.getSide(box) == CPlane::BOTH_SIDE);
	}
	//-----------------------------------------------------------------------
	bool Math::intersects(const CSphere& sphere, const CPlane& plane)
	{
		return (
	...
	//-----------------------------------------------------------------------
	CVector3 Math::calculateTangentSpaceVector(
		const CVector3& position1, const CVector3& position2, const CVector3& position3,
		Real u1, Real v1, Real u2, Real v2, Real u3, Real v3)
	{
		//side0 is the vector along one side of the triangle of vertices passed in, 
		//and side1 is the vector along another side. Taking the cross product of these returns the normal.
		CVector3 side0 = position1 - position2;
		CVector3 side1 = position3 - position1;
		//Calculate face normal
		CVector3 normal = side1.crossProduct(side0);
		normal.normalise();
		//Now we use a formula to calculate the tangent. 
		Real deltaV0 = v1 - v2;
		Real deltaV1 = v3 - v1;
		CVector3 tangent = deltaV1 * side0 - deltaV0 * side1;
		tangent.normalise();
		//Calculate binormal
		Real deltaU0 = u1 - u2;
		Real deltaU1 = u3 - u1;
		CVector3 binormal = deltaU1 * side0 - deltaU0 * side1;
		binormal.normalise();
		...
		CVector3 tangentCross = tangent.crossProduct(binormal);
	...
	//-----------------------------------------------------------------------
	CMatrix4 Math::buildReflectionMatrix(const CPlane& p)
	{
		return CMatrix4(
	...
	//-----------------------------------------------------------------------
	CVector4 Math::calculateFaceNormal(const CVector3& v1, const CVector3& v2, const CVector3& v3)
	{
		CVector3 normal = calculateBasicFaceNormal(v1, v2, v3);
		// Now set up the w (distance of tri from origin
		return CVector4(normal.x, normal.y, normal.z, -(normal.dotProduct(v1)));
	}
	//-----------------------------------------------------------------------
	CVector3 Math::calculateBasicFaceNormal(const CVector3& v1, const CVector3& v2, const CVector3& v3)
	{
		CVector3 normal = (v2 - v1).crossProduct(v3 - v1);
		normal.normalise();
		return normal;
	}
	//-----------------------------------------------------------------------
	CVector4 Math::calculateFaceNormalWithoutNormalize(const CVector3& v1, const CVector3& v2, const CVector3& v3)
	{
		CVector3 normal = calculateBasicFaceNormalWithoutNormalize(v1, v2, v3);
		// Now set up the w (distance of tri from origin)
		return CVector4(normal.x, normal.y, normal.z, -(normal.dotProduct(v1)));
	}
	//-----------------------------------------------------------------------
	CVector3 Math::calculateBasicFaceNormalWithoutNormalize(const CVector3& v1, const CVector3& v2, const CVector3& v3)
	{
		CVector3 normal = (v2 - v1).crossProduct(v3 - v1);
		return normal;
	}
	...
	//---------------------------------------------------------------------
	CMatrix4 Math::makeViewMatrix(const CVector3& position, const CQuaternion& orientation, 
		const CMatrix4* reflectMatrix)
	{
		CMatrix4 viewMatrix;
		...
		CVector3 trans = -rotT * position;

		// Make final matrix
		viewMatrix = CMatrix4::IDENTITY;
	...
	//---------------------------------------------------------------------
	Real Math::boundingRadiusFromAABB(const CAxisAlignedBox& aabb)
	{
		CVector3 max = aabb.getMaximum();
		CVector3 min = aabb.getMinimum();

		CVector3 magnitude = max;
